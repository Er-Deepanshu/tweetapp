  ...
  useEffect(() => {
    ...
    async function fetchMainTweet() {
      try {
        ...
        const response = await axios.get(`http://ec2-54-87-169-244.compute-1.amazonaws.com:8080/api/v1.0/tweets/main/${tweetId}`)
      ...
      replies.forEach(reply => {
        async function fetchReplies() {
          try {
           ...
           const response = await axios.get(`http://ec2-54-87-169-244.compute-1.amazonaws.com:8080/api/v1.0/tweets/main/${reply.id}`)
  ...
  return (
    <div className="container p-3">
      {mainTweet ? (
        ...
        <div className="text-center">Sorry! This Tweet is unavailable.</div>
      )}
    </div>
  )
}
